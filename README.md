...
当前 README 默认以 **DeepSeek 官方 API** 为示例，建议先按默认配置跑通；初始化向导也支持选择 **OpenRouter**，默认模型为 `openrouter/owl-alpha`。

- 🌐 打开 [DeepSeek 平台](https://platform.deepseek.com/)
- 📝 完成注册 / 登录
- 🔐 充值并创建密钥

如使用 OpenRouter，请在初始化向导中选择 OpenRouter，并填写 OpenRouter API Key。
...
- **OpenRouter**：提供可选的 OpenAI-compatible LLM API 入口，可在初始化向导中选择并使用 `openrouter/owl-alpha`。
